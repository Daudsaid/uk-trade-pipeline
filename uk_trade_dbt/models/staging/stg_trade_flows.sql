with trade_flows as (
    select * from {{ source('trade_db', 'trade_flows') }}
),
dim_countries as (
    select * from {{ source('trade_db', 'dim_countries') }}
),
dim_commodities as (
    select * from {{ source('trade_db', 'dim_commodities') }}
),
dim_ports as (
    select * from {{ source('trade_db', 'dim_ports') }}
)
select
